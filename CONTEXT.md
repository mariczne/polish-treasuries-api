...
Japanese conventions for the same constructs.
...
_Avoid_: retail bond, detaliczne
...
_Avoid_: T-bond (alone), hurtowe, market bond, linker
...
_Avoid_: interest period, year (alone), kupon, okres odsetkowy
...
_Avoid_: interest rate, yield, oprocentowanie, coupon (for the rate)
...
discount).
_Avoid_: price (alone), cena emisyjna
...
_Avoid_: redemption date (for the rule), tenor (for the date), data wykupu
...
_Avoid_: WRₖ (the letter's symbol), CPI m/m, monthly inflation, GUS CPI
...
_Avoid_: Wₙ, index (alone), inflation index, cumulative index, Reference CPI
...
_Avoid_: WB, base index, base CPI, inflation base
...
_Avoid_: SI, index ratio, indexation (alone), coefficient (alone)
...
_Avoid_: WZₙ, replacement index, fallback
